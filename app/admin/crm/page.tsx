...
export default function UnifiedCRMDashboard() {
  ...
  const handleEditLead = (lead: Lead) => {
    setEditFormData({
      ...
      email: lead.email,
      phone: lead.phone,
      sources: Array.isArray(lead.source) ? lead.source : (lead.source ? [lead.source] : []) // FIX APPLIED HERE
    })
    setShowEditForm(true)
  }

  // FIXED FUNCTION: handleUpdateLead
  const handleUpdateLead = async () => {
    if (!editFormData || !editFormData.name || !editFormData.company || !editFormData.value) {
      alert('Please fill in all required fields!')
      return
